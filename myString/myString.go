...
func test_topic() {
	topic := "com.zte.zpush/890455137"
	delimiter := strings.Index(topic, "/")
	appname, appkey := topic[0:delimiter], topic[delimiter+1:]
	fmt.Println("appname:", appname, ",appkey:", appkey)
}
...
func test_basic_methods() {
	...
	org := "NLT_abc"
	index := strings.Index(org, "abc") // 返回第一个匹配字符的位置，这里是4
	fmt.Println(index)
	// str := "NLT_abc"[index:] // 这种写法也可以
	str := org[index:]
